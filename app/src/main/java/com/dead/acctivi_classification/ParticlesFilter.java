...
public class ParticlesFilter {
    ...
    private  int Ns; // number of particles
    public Particles[] particles;
    private FloorPlan floorPlan;
    public Position currentPosition;

    public ParticlesFilter(int Ns) {
        this.floorPlan = new FloorPlan();
        this.Ns = Ns;
        this.particles = new Particles[Ns];
        ...
        for (Room room : floorPlan.rooms) {
            ...
            while (particleIdx < numberOfParticles && particleIdx < Ns) {
                ...
                double stride = (STRIDE_MIN + STRIDE_MAX) / 2.0 + (STRIDE_MAX - STRIDE_MIN) * (rngStride.nextDouble()
                        - 0.5);
                particles[particleIdx] = new Particles(position, stride, weight);
                particleIdx++;
            }
    ...
    public void moveParticles(double stepCount, double azimuth) {
        ...
        for (Particles particle : particles) {
            ...
            double stride = stepCount * step_size;
    ...
    public void resampleParticles() {
        Particles[] resampledParticles = new Particles[Ns];
        ...
        for (int i = 0; i < Ns; i++) {
            ...
            if (particles[cdf_idx].getWeight() == 0.0)
                resampledParticles[i] = new Particles(resampledParticles[i - 1]);
            else
                resampledParticles[i] = new Particles(particles[cdf_idx]);

...
public class ParticleFilter  {
    ...
    public void move(float turn, float movement) throws Exception {
        for (int i = 0; i < numParticles; i++) {
            particles[i].move(turn, movement);
        }
    }
    ...
    public void resample(float[] measurement) throws Exception {
        ...
        for (int i = 0; i < numParticles; i++) {
            ...
            new_particles[i].set(particles[index].x, particles[index].y, particles[index].orientation, (float) particles[index].probability);
